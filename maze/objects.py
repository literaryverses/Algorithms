...
class Grid:  # orthogonal maze (shape == 4) by default
    def __init__(self, rows: int, columns: int, levels=1, shape=4):
        ...
        self.lvls = levels
        self.grid = [
            [[0 for c in range(columns)] for r in range(rows)] for l in range(levels)
        ]
        self.borders = {
            "north": "---",
            "south": "---",
    ...
    def _drawCell(self, row_layout, cell):
        ...
        middle += (
            f"{self._drawBorder(cell, 'west')} {self._drawBorder(cell, 'within')} "
        )
    ...
    def __drawWithin(self, cell: Cell) -> str:
        ...
        for i, direction in enumerate(directions):
            if (
                direction in cell.neighbors.keys()
                and cell.isLinked(cell.neighbors[direction])
                and cell.neighbors[direction] != None
            ):
        ...
        if directions[0] and directions[1]:
            ...
            return self.borders["up"]
        elif directions[1]:
            return self.borders["down"]
        else:
            return
    ...
    def _drawBorder(self, cell: Cell, direction: str) -> str:
        if direction == "within":
            if within := self.__drawWithin(cell):
                return within
        elif direction in cell.neighbors.keys() and not cell.isLinked(
    ...
    def _drawCorner(self, y: int, x: int, z=0) -> str:  # given two diagonal cells
        ...
        for order, make_cell in enumerate(adjacents):
            ...
            if make_cell:
                ...
                if not cell.isLinked(cell.neighbors[skews[long]]) or not cell.isLinked(
                    cell.neighbors[skews[lat]]
                ):
                    return "+"
        return " "
    ...
    def mask(self, row: int, col: int, lvl=0):  # disconnects cell from rest of grid
        ...
        for neighbor in cell.getNeighbors(False):
            if (
                neighbor.isMasked()
                and neighbor != cell.neighbors.get("up")
                and neighbor != cell.neighbors.get("down")
            ):  # prevent z-axis linking between masked cells
                cell.link(neighbor)
...
class TriGrid(Grid):  # delta grid (shape = 3)
    ...
    def _drawCell(self, row_layout, cell):
        ...
        if sum(cell.coord[:-1]) % 2:
            ...
            if (
                cell.coord[1] == 0 and cell.coord[0] == self.rows - 1
            ):  # last cell in 1st column
                bottom += "  "  # indentation
            if sum(cell.coord[:-1]) == self.cols + self.rows - 2:  # last cell
            ...
            if cell.coord[1] == 0:  # 1st column
                top += (
                    f"  {self._drawCorner(cell.coord[0], cell.coord[1], cell.coord[2])}"
                )
                middle += f" {self._drawBorder(cell, 'northwest')}"
            if cell.coord[0] == self.rows - 1:  # last row
                bottom += self._drawCorner(
                    cell.coord[0] + 1, cell.coord[1] - 1, cell.coord[2]
                )
    ...
    def _drawCorner(self, y: int, x: int, z=0) -> str:  # given two diagonal cells
        ...
        for order, make_cell in enumerate(adjacents):
            if make_cell and order == 0:
                ...
                if not downMid.isLinked(
                    downMid.neighbors["northwest"]
                ) or not downMid.isLinked(downMid.neighbors["northeast"]):
                    return "x"
            elif make_cell and order == 1:
                downRight = self.getCell(y, x + 1, z)
                if not downRight.isLinked(
                    downRight.neighbors["north"]
                ) or not downRight.isLinked(downRight.neighbors["southwest"]):
                    return "x"
            elif make_cell and order == 2:
                upRight = self.getCell(y - 1, x + 1, z)
                if not upRight.isLinked(
                    upRight.neighbors["south"]
                ) or not upRight.isLinked(upRight.neighbors["northwest"]):
                    return "x"
            elif make_cell and order == 3:
                upMid = self.getCell(y - 1, x, z)
                if not upMid.isLinked(
                    upMid.neighbors["southeast"]
                ) or not upMid.isLinked(upMid.neighbors["southwest"]):
                    return "x"
            elif make_cell and order == 4:
                upLeft = self.getCell(y - 1, x - 1, z)
                if not upLeft.isLinked(
                    upLeft.neighbors["northeast"]
                ) or not upLeft.isLinked(upLeft.neighbors["south"]):
                    return "x"
            elif make_cell and order == 5:
                downLeft = self.getCell(y, x - 1, z)
                if not downLeft.isLinked(
                    downLeft.neighbors["north"]
                ) or not downLeft.isLinked(downLeft.neighbors["southeast"]):
                    return "x"
        return " "
...
class HexGrid(Grid):  # sigma maze (shape = 6)
    ...
    def __reformat(self, hex_cell: Cell, triGrid: TriGrid) -> TriGrid:
        for direction in hex_cell.neighbors.keys():
            ...
            if neighbor and hex_cell.isLinked(neighbor):
                ...
                triangle = self.__getTriangle(
                    hex_cell, "up", triGrid
                )  # get corresp triangle
                self.__maskUp(
                    triGrid,
                    False,
                    triangle.coord[0],
    ...
    def __getTriangle(self, hex_cell: Cell, direction: str, triGrid: TriGrid) -> Cell:
        ...
        return triGrid.getCell(
            2 * row + y_adjust(col, direction), 3 * col + x_adjust(direction), lvl
        )
